...
class StationsViewController: UIViewController {
  ...
  override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
    if segue.identifier == "NowPlaying", let nowPlayingVC = segue.destination as? NowPlayingViewController {
    ...
    else if segue.identifier == "History", let historyVC = segue.destination as? HistoryViewController {
      
      historyVC.trackHistory = self.trackHistory
    }
  }
...
extension StationsViewController: RadioPlayerDelegate {
  ...
  func trackDidUpdate(_ track: Track?) {
    ...
    if let currentTrack = track, currentTrack.title != radioPlayer.station?.name
    {
      if trackHistory.count > 0, currentTrack.title == trackHistory[0].track.title { return }
      
      trackHistory.insert((currentTrack, Date()), at: 0)
    }
  }
